...
function sendConfirmationEmails_(ss, registrationId, data, config, rowIndex, triggerName) {
  ...
  recipients.forEach(function(recipient) {
    const subject = '[AI SHOWCASE 2026] Đã nhận đăng ký · ' + data.usecaseName;
    ...
    const plainBody =
      ...
      'BTC đã nhận đăng ký AI Showcase 2026 của bạn.\n\n' +
      'Mã đăng ký: ' + registrationId + '\n' +
      'Hình thức: ' + data.mode + '\n' +
      'Use Case: ' + data.usecaseName + '\n' +
      'AI Showcase dự kiến: ' + (config.SHOWCASE_DATE || '25/09/2026') + '\n\n' +
      'Email này xác nhận BTC đã nhận dữ liệu, chưa phải thông báo kết quả đánh giá/shortlist.\n' +
      'BTC sẽ liên hệ thêm qua email hoặc Telegram khi có cập nhật tiếp theo.';
...
function testConfirmationEmail() {
  ...
  MailApp.sendEmail({
    to: recipient,
    subject: '[TEST] AI Showcase 2026 · Email confirmation',
    body:
